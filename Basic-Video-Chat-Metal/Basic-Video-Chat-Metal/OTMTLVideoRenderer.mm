...
@implementation OTMTLVideoRenderer {
...
- (void)getWidth:(nonnull int *)width
     ...
     scalesToFit:(nonnull bool *)scalesToFit
       mirroring:(nonnull bool *)mirroring
         ofFrame:(nonnull otc_video_frame *)frame {
    
    *width = otc_video_frame_get_width(frame);
    *height = otc_video_frame_get_height(frame);
    *cropWidth = otc_video_frame_get_width(frame);
    *cropHeight = otc_video_frame_get_height(frame);
    *cropX = 0;
    *cropY = 0;
...
- (BOOL)setupTexturesForFrame:(otc_video_frame *)frame
    ...
    // Luma (y) texture.
    if (!_descriptor || _width != otc_video_frame_get_width(frame)
                         || _height != otc_video_frame_get_height(frame) ||
                         CGSizeEqualToSize(_viewSizeOld, viewSize) == false) {
        _width = otc_video_frame_get_width(frame);
        _height =otc_video_frame_get_height(frame);
    ...
    [_yTexture replaceRegion:MTLRegionMake2D(0, 0, _width, _height)
                 mipmapLevel:0
                   withBytes:otc_video_frame_get_plane_binary_data(frame,OTC_VIDEO_FRAME_PLANE_Y)
                 bytesPerRow:otc_video_frame_get_plane_stride(frame,OTC_VIDEO_FRAME_PLANE_Y)];
    if (!_chromaDescriptor ||
        (_chromaWidth != ((otc_video_frame_get_width(frame) + 1) / 2) ||
         _chromaHeight != ((otc_video_frame_get_height(frame) + 1) / 2)))
    {
        _chromaWidth = (otc_video_frame_get_width(frame) + 1) / 2;
        _chromaHeight = (otc_video_frame_get_height(frame) + 1) / 2;
        _chromaDescriptor =
    ...
    [_uTexture replaceRegion:MTLRegionMake2D(0, 0, _chromaWidth, _chromaHeight)
                 mipmapLevel:0
                   withBytes:otc_video_frame_get_plane_binary_data(frame,OTC_VIDEO_FRAME_PLANE_U)
                 bytesPerRow:otc_video_frame_get_plane_stride(frame,OTC_VIDEO_FRAME_PLANE_U)];
    [_vTexture replaceRegion:MTLRegionMake2D(0, 0, _chromaWidth, _chromaHeight)
                 mipmapLevel:0
                   withBytes:otc_video_frame_get_plane_binary_data(frame,OTC_VIDEO_FRAME_PLANE_V)
                 bytesPerRow:otc_video_frame_get_plane_stride(frame,OTC_VIDEO_FRAME_PLANE_V)];
...
- (void)drawFrame:(otc_video_frame *)frame viewSize:(CGSize)viewSize {
    @autoreleasepool {
